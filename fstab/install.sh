...
CACHE_FILE="${CACHE_ROOTFS_DIR}/ubuntu-fstab-${FSTAB_CACHE_HASH}.tar.gz"
...
if [ -f "${CACHE_FILE}" ]; then
    echo "Found cached fstab package, extracting..."
    sudo rm -rf "${ROOTFS}"
    sudo mkdir -p "${ROOTFS}"
    ...
    if [ ! -d "${ROOTFS}/var/cache/apt/archives" ]; then
        restore_from_first_available_stage base || true
    fi
    ...
    sudo chroot "${ROOTFS}" /bin/bash -c "
        ...
        dpkg --configure -a
        apt-get install -f -y ${CONFIG_UBUNTU_FSTAB}
        #systemctl mask serial-getty@ttyFIQ0.service
        sync
    "
