...
CACHE_FILE="${CACHE_ROOTFS_DIR}/ubuntu-desktop-${DESKTOP_CACHE_HASH}.tar.gz"
...
if [ -f "${CACHE_FILE}" ]; then
    echo "Found cached desktop package, extracting..."
    sudo rm -rf "${ROOTFS}"
    mkdir -p "${ROOTFS}"
    ...
    if [ ! -d "${ROOTFS}/var/cache/apt/archives" ]; then
        restore_from_first_available_stage app fstab base || true
    fi
    ...
    sudo chroot "${ROOTFS}" /bin/bash -c "
        ...
        dpkg --configure -a
        apt-get install -f -y ${CONFIG_UBUNTU_DESKTOP}
        sync
    "
